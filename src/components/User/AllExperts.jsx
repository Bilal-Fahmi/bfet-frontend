...
export default function AllExperts() {
  ...
  const fetchData = async () => {
    try {
      const res = await apiInstance.get("/allexp");
  ...
  return (
    <div className=" flex flex-row mr-auto p-10 ">
      <h1 className="extrabold mr-10 text-2xl text-black">
        Talk <br /> to an <br />
        Expert.
      </h1>
        ...
        <div className="pl-5" key={index}>
          ...
          >
            ...
            <CardFooter className="absolute bg-white/30 bottom-0 border-t-1 border-zinc-100/50 z-10 justify-between">
              <div>
                <p
                  className={`text-white semibold rounded p-1    bg-[#5AA17F] bg-opacity-50 text-md ${
                    expert.selectedOption === "Mind"
