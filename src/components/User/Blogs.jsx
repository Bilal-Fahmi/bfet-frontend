      ...
      {blog.map((blog, index) => (
        ...
        >
          ...
          <CardFooter className="text-small medium justify-between">
            <b>{blog.title}</b>
            <p className="text-default-500 light">{format(new Date(blog.createdAt),'MMM d,yyyy HH:mm')}</p>

          </CardFooter>
          <CardFooter >
            <p className="light text-small flex-col">{turncatText(blog.summary, 100)}</p>
          </CardFooter>
        </Card>
      ))}
      </div>
